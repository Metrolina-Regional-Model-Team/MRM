...
//////
	...
	Opts = RunMacro("create_tnet", "peak", "bus", "drive", Dir)
...
// This new macro penalizes drive access time if driving to a PNR lot farther then closes formal PNR lot. It also adds the shadow price.
	...
	rtn_OP = RunMacro("Compute_OP_Matrix", "peak", "bus", "drive", Args)
	if rtn_OP[1] = 0
		then do
			Throw(rtn_OP[2])
			// msg = msg + rtn_OP[2]
...
// ----------------------------------- STEP 2: Transit Network Setting  -----------------------------------
	...
	Opts = RunMacro("set_tnet", "peak", "bus" , "drive", Dir)
...
// ----------------------------------- STEP 3: Transit Skim Path Finder  -----------------------------------
     ...
     Opts.Input.Network = Dir + "\\PbusD.tnw"
...
// added by JainM Dec 05
       ...
       rtn_upskim = RunMacro("Update_Drive_Skims_Mtx", "peak", "bus", "drive", Args)
