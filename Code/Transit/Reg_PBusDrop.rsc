...
// 5/30/19, mk: There are now three distinct networks, use offpeak since was used to setup route system
    ...
    AppendToLogFile(1, "Enter Reg_PBusDrop: " + datentime)
...
// ----------------------------------- STEP 3a: Transit Skim Path Finder  -----------------------------------
     ...
     Opts.Input.Network = Dir + "\\PBusDrop.tnw"
     ...
     Opts.Output.[Skim Matrix].[File Name] = Dir + "\\skims\\TR_SKIM_PBusDrop.mtx"
     Opts.Output.[Parking Matrix].Label = "TR_PARK_PBusDrop"
     Opts.Output.[Parking Matrix].[File Name] = Dir + "\\skims\\TR_PARK_PBusDrop.mtx"
// -------- Output Origin-to-Drop-Off Time Matrix ---------------
//     Opts.Output.[OP Matrix].Label = "OP Time Matrix (Drop-Off-Peak)"
//     Opts.Output.[OP Matrix].[File Name] = Dir + "\\skims\\TR_OP_SKIM_PBusDrop.mtx"
...
// ----------------------------------- STEP 3b: Transit Skim Path Finder (No CBD Drop-Off) ------------------
    ...
    if !ret_value then goto badtransettings
     ...
     Opts.Global.[Skim Modes] = { 5, 6 , 7, 8, 9, 10, 11}
     Opts.Output.[Skim Matrix].Label = "Skim Matrix (Pathfinder)"
     Opts.Output.[Skim Matrix].[File Name] = Dir + "\\skims\\TR_SKIM_PBusDrop_NoCBD.mtx"
     Opts.Output.[Parking Matrix].Label = "TR_PARK_PBusDrop_NoCBD"
     Opts.Output.[Parking Matrix].[File Name] = Dir + "\\skims\\TR_PARK_PBusDrop_NoCBD.mtx"
...
//      end
     ...
     matx1 = OpenMatrix(Dir + "\\skims\\TR_SKIM_PBusDrop.mtx", "FALSE")
     midx1 = GetMatrixIndex(matx1)
     core1 = GetMatrixCoreNames(matx1)

     matx2 = OpenMatrix(Dir + "\\skims\\TR_SKIM_PBusDrop_NoCBD.mtx", "FALSE")
     midx2 = GetMatrixIndex(matx2)
     core2 = GetMatrixCoreNames(matx2)

     matx3 = OpenMatrix(Dir + "\\skims\\TR_PARK_PBusDrop.mtx", "FALSE")
     midx3 = GetMatrixIndex(matx3)
     core3 = GetMatrixCoreNames(matx3)

     matx4 = OpenMatrix(Dir + "\\skims\\TR_PARK_PBusDrop_NoCBD.mtx", "FALSE")
...
//  om = OpenMatrix(Dir + "\\skims\\TR_PARK_pbusdrop.mtx", "True")
//  CreateTableFromMatrix(om, Dir + "\\tranassn\\pbusdrop\\pnrnode_pbusdrop.asc", "FFA", {{"Complete", "Yes"}})
    ...
    rtn_Pflags = RunMacro("Create PbusDrop Parking Flags", Args)
...
// -- process Skim matrix
    ...
    OM = OpenMatrix(Dir + "\\skims\\TR_SKIM_PBusDrop.mtx", "True")
...
//--- include the highway skim matrix as a part of Transit Skim Matrices 
    ...
    M1 = OpenMatrix(Dir+"\\skims\\TR_SKIM_PBusDrop.mtx", "True")
...
// --- Update the Skim Matrix With the Park Flag
    ...
    park_matrix = Dir + "\\Skims\\ParkFlag_PbusDrop.mtx"

    M1 = OpenMatrix(park_matrix, "True")
    M2 = OpenMatrix(Dir+"\\skims\\TR_SKIM_PBusDrop.mtx", "True")
...
if (theyear = "2009") then do
     ...
     Opts = {{"Input",    {{"Matrix Currency",   {Dir + "\\skims\\TR_SKIM_PBusDrop.mtx",
                                                  "InVehGT5",
                                                  "RCIndex",
                                                  "RCIndex"}}}},
             {"Global",   {{"Method",            11},
...
Opts = null
     ...
     ret_value = RunMacro("TCB Run Operation", 28, "Fill Matrices", Opts)
    if !ret_value then goto badmatrixop
...
// added by JainM March 07
       ...
       rtn_upskim = RunMacro("Update_DropOff_Skims_Mtx", "peak", "bus", "dropoff", Args)
...
//  status = RunProgram(pkzip_program + " " + Dir+"\\skims\\TR_SKIM_pbusdrop.zip " +Dir+"\\skims\\TR_Skim_pbusdrop.mtx -Add",{{"Minimize", "True"}})

//  if status = 0 then DeleteFile(Dir+"\\skims\\TR_Skim_pbusdrop.mtx")

    badcomputeopmatrix:
    Throw("Reg_PBusDrop - Error return from Compute_OP_Matrix")
    // Throw("Reg_PBusDrop - Error return from Compute_OP_Matrix")
    // AppendToLogFile(1, "Reg_PBusDrop - Error return from Compute_OP_Matrix") 
    // goto badquit

    badupskim:
    Throw("Reg_PBusDrop - Error return from Update_Dropoff_Skim_Mtx")
    // msg = rtn_upskim[2] + {"Reg_PBusDrop - Error return from Update_Dropoff_Skim_Mtx"}
    // AppendToLogFile(1, "Reg_PBusDrop - Error return from Update_Dropoff_Skim_Mtx") 
    // goto badquit
        
    badparkflags:
    Throw("Reg_PBusDrop - Error return from Create PPrmDrop Parking Flags")
    // msg = rtn_Pflags[2] + {"Reg_PBusDrop - Error return from Create PPrmDrop Parking Flags"}
    // AppendToLogFile(1, "Reg_PBusDrop - Error return from Create PPrmDrop Parking Flags") 
    // goto badquit

    badbuildtrannet:
    Throw("Reg_PBusDrop - Error return build transit network")
    // Throw("Reg_PBusDrop - Error return build transit network")
    // AppendToLogFile(1, "Reg_PBusDrop - Error return build transit network") 
    // goto badquit

    badtransettings:
    Throw("Reg_PBusDrop - Error return from transit network settings")
    // Throw("Reg_PBusDrop - Error return from transit network settings")
    // AppendToLogFile(1, "Reg_PBusDrop - Error return from transit network settings") 
    // goto badquit

    badtranskim:
    Throw("Reg_PBusDrop - Error return from transit network skims")
    // Throw("Reg_PBusDrop - Error return from transit network skims")
    // AppendToLogFile(1, "Reg_PBusDrop - Error return from transit network skims")
    // goto badquit

    badmatrixop:
    Throw("Reg_PBusDrop - Error in matrix operations")
    // Throw("Reg_PBusDrop - Error in matrix operations")
    // AppendToLogFile(1, "Reg_PBusDrop - Error in matrix operations")
    // goto badquit
...
quit:
    ...
    datentime = GetDateandTime()
    AppendToLogFile(1, "Exit Reg_PBusDrop: " + datentime)
    AppendToLogFile(1, " ")

    return({RegPBusDropOK, msg})
...
// -----------------------------------------
//   Macro "Create PbusDrop Parking Flags"
// -----------------------------------------

Macro "Create PbusDrop Parking Flags" (Args)
...
shared route_file, routename, net_file, link_lyr, node_lyr, parking_view, nodes_view
    ...
    ParkFlagsOK = 1
    datentime = GetDateandTime()
    AppendToLogFile(2, "Enter Create PBusDrop Parking Flags: " + datentime)

m = OpenMatrix(Dir + "\\skims\\TR_PARK_PBusDrop.mtx", )
mc1 = CreateMatrixCurrency(m, "Parking Nodes", "RCIndex", "RCIndex", )
mc2 = CreateMatrixCurrency(m, "Parking Nodes", "RCIndex", "RCIndex", )
CopyMatrixStructure({mc1,mc2}, {{"File Name", Dir + "\\skims\\ParkFlag_PbusDrop.mtx"},
    {"Label", "New Matrix"},
    {"File Based", "Yes"},
    {"Tables", {"Parking_Flag"}},
    {"Operation", "Union"},
...
/*
    ...
    WriteLine(ctl, DirSlash + "//Skims//TR_PARK_PBusDrop.mtx")
    WriteLine(ctl, DirSlash + "//skims//KNR_CAT.asc")
    WriteLine(ctl, DirSlash + "//Skims//ParkFlag_PbusDrop.mtx")
    CloseFile(ctl)
    ...
    AppendToLogFile(2, "Create PBusDrop Parking Flags call to fortran: pgm=\\ModeChoice\\KNR_Loc_CAT.exe, timestamp: " + TimeStamp)

     status = RunProgram(batchname,{{"Maximize", "True"}})
    if (status <> 0) then goto badfortran
*/
    ascfile = DirSlash + "//skims//KNR_CAT.asc"
    inmtx = DirSlash + "//Skims//TR_PARK_PBusDrop.mtx"
    outmtx = DirSlash + "//Skims//ParkFlag_PbusDrop.mtx"
    RunMacro("Run KCAT", ascfile, inmtx, outmtx)

    modesplit_matrix = Dir + "\\skims\\PK_DROPTRAN_SKIMS.MTX"
    input_matrix= Dir + "\\skims\\ParkFlag_PbusDrop.mtx"
     Opts = null
    ...
    badmerge:
    Throw("Create PBusDrop Parking Flags - Error merging matrices")
    // Throw("Create PBusDrop Parking Flags - Error merging matrices")
    // AppendToLogFile(1, "Create PBusDrop Parking Flags - Error merging matrices")
    // goto badquit
    ...
    quit:
    datentime = GetDateandTime()
    AppendToLogFile(2, "Exit Create PBusDrop Parking Flags: " + datentime)
    AppendToLogFile(1, " ")
